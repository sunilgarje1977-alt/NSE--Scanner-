        ...
        try:
            ...
            if pos['type'] == 'BUY':
                if ltp >= pos['t2'] and pos['sl'] < pos['t1']:
                    pos['sl'] = pos['t1']
                    send_tg(f"TRAIL {pos['symbol']} SL to T1 {pos['t1']}")
                elif ltp >= pos['t1'] and pos['sl'] < pos['entry']:
                    pos['sl'] = pos['entry']
                    send_tg(f"TRAIL {pos['symbol']} SL to COST")
                if ltp <= pos['sl']:
                    pnl = ltp - pos['entry']
                    daily['pnl'] += pnl
                    daily['trades'] += 1
                ...
                if ltp <= pos['t2'] and pos['sl'] > pos['t1']:
                    pos['sl'] = pos['t1']
                    send_tg(f"TRAIL {pos['symbol']} SL to T1")
                elif ltp <= pos['t1'] and pos['sl'] > pos['entry']:
                    pos['sl'] = pos['entry']
                    send_tg(f"TRAIL {pos['symbol']} SL to COST")
                if ltp >= pos['sl']:
...
def scan_one(sym):
    ...
    try:
        ...
        last = df.iloc[-1]
        prev = df.iloc[-2]
        # BUY - Breakout + Green + Vol + EMA Cross + VWAP UP + RSI 60
        if last['close'] > df['high20'].iloc[-2] and last['close'] > last['open'] and last['volume'] > last['vol_avg']*1.2 and last['ema9'] > last['ema15'] and prev['ema9'] <= prev['ema15'] and last['close'] > last['vwap'] and last['rsi'] >= 60:
            ...
            if risk > 0:
                entry = last['close']
                return {"symbol":sym,"type":"BUY","entry":entry,"sl":last['low'],"t1":entry+risk,"t2":entry+risk*2,"t5":entry+risk*5,"rsi":last['rsi'],"vol":last['volume']/last['vol_avg']}
        # SELL - Breakdown + Red + Vol + EMA Cross Down + VWAP DN + RSI 40
        if last['close'] < df['low20'].iloc[-2] and last['close'] < last['open'] and last['volume'] > last['vol_avg']*1.2 and last['ema9'] < last['ema15'] and prev['ema9'] >= prev['ema15'] and last['close'] < last['vwap'] and last['rsi'] <= 40:
            ...
            if risk > 0:
                entry = last['close']
                return {"symbol":sym,"type":"SELL","entry":entry,"sl":last['high'],"t1":entry-risk,"t2":entry-risk*2,"t5":entry-risk*5,"rsi":last['rsi'],"vol":last['volume']/last['vol_avg']}
    except:
        return None
    return None

send_tg(f"FAST BOT V3 STARTED - 1000 Stocks - 2 Active - 6 Daily - 1 Min - 1:2:5 Trail")

for _ in range(360):
    active, daily = check_pnl()
    if daily['trades'] >= 6:
        send_tg(f"DAILY LIMIT 6 DONE PnL {daily['pnl']:.1f} Wins {daily['wins']} Loss {daily['loss']}")
        break
    if len(active) < 2:
        with ThreadPoolExecutor(max_workers=60) as ex:
            ...
            for f in as_completed(futs):
                ...
                if r and r['symbol'] not in [a['symbol'] for a in active]:
                    ...
                    msg = f"NEW {r['type']} {r['symbol']} @ {r['entry']:.1f} SL {r['sl']:.1f} T1 {r['t1']:.1f} T2 {r['t2']:.1f} T5 {r['t5']:.1f} RSI {r['rsi']:.0f} Vol {r['vol']:.1f}x Active {len(active)}/2 Daily {daily['trades']}/6 PnL {daily['pnl']:.1f}"
                    send_tg(msg)
                    if len(active) >= 2:
                        break
    print(f"{datetime.now().strftime('%H:%M')} Active {len(active)}/2 Daily {daily['trades']}/6 PnL {daily['pnl']}")
    time.sleep(60)

daily = load_json(DAILY_FILE, {})
send_tg(f"FINAL REPORT {daily.get('date')} Trades {daily.get('trades')}/6 Wins {daily.get('wins')} Loss {daily.get('loss')} PnL {daily.get('pnl',0):.1f}")
